 ...
 * ============================================================================
 *  MAIN.JS
 *  Haalt de actuele inhoud op bij /api/config (beheerd via het admin-paneel
 *  op /admin) en bouwt daarmee de pagina op. Regelt ook de interactie
...
(function () {
  async function laadConfig() {
    ...
    if (!res.ok) throw new Error("Kon configuratie niet ophalen: " + res.status);
    return res.json();
  }

  // ---------------------------------------------------- 1. Huisstijl toepassen
  function pasHuisstijlToe(config) {
  ...
  // ---------------------------------------------------- 2. SEO / metadata
  function pasMetadataToe(config) {
  ...
  // ---------------------------------------------------- 6. Contactformulier
  function initContactformulier(config) {
    ...
    function wisVeldFout(veld) {
      veld.closest(".formulier-veld").classList.remove("heeft-fout");
    }
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      document.body.innerHTML =
        '<p style="font-family:sans-serif;padding:3rem;max-width:40rem;margin:0 auto;">' +
        "De website-inhoud kon niet worden geladen. Probeer de pagina te verversen, " +
        "of controleer of de server draait.</p>";
      return;
    }

    pasHuisstijlToe(config);
    pasMetadataToe(config);
    bouwPagina(config);
    initNavigatie();
    initFaq();
    initContactformulier(config);
  });
})();
